...
%}
train_data_address = input('File Name of Training Data(in CSV format): ', 's');
test_data_address = input('File Name of Test Data(in CSV format): ', 's');
alpha = input('Learning Rate(alpha): ');
epoch = input('Epoch: ');
...
for i = 2:size(X, 2)
    featureNum = i - 1;
    figure('Name', strcat('Overall Score - Feature x', int2str(featureNum)));
